...
// url: http://stackoverflow.com/questions/19094340/stdvector-as-a-template-function-argument
template<typename T, typename A>
void cout_vector(std::vector<T,A> const& vec, const std::string &id_name="") {
    cout    << "- array of " << id_name << " -" << std::endl << "[" \
                << join(vec.begin(), vec.end(), std::string(",")) \
                << std::endl << "]" << std::endl;
}
...
public:
    ...
    virtual void startPart(int instrCount) { cout << "startPart with " << instrCount << " instrument(s)" << endl; }
    ...
    virtual void newNote(long date, int chan, float pitch, int vel, int dur, int fret, int string,
                         int string_mute) {
        //cout << date << " newNote [" << chan << "," << pitch << "," << vel << "," << dur << "," << fret << "," << string <<
        //"," << string_mute << "]" << endl;
    ...
    virtual void tempoChange(long date, int bpm) { cout << date << " tempoChange " << bpm << endl; }
