...
//________________________________________________________________________
void scoreInstrument::reset() { fID = fName = fAbbreviation = ""; }
...
void scoreInstrument::visitStart(S_instrument_name& elt) {
  fName = elt->getValue();
}
void scoreInstrument::visitStart(S_instrument_abbreviation& elt) {
  fAbbreviation = elt->getValue();
}
...
void midiInstrument::visitStart(S_midi_name& elt) {
  fMIDIName = elt->getValue();
}
...
void midiInstrument::visitStart(S_midi_unpitched& elt) {
  fUnpitched = (int)(*elt);
}
...
//________________________________________________________________________
void midicontextvisitor::playPedalChange(midiwriter::pedalType type,
...
void midicontextvisitor::playTempoChange(long bpm) {
  if (fMidiWriter && bpm) fMidiWriter->tempoChange(fCurrentDate, bpm);
}
...
void midicontextvisitor::playScoreInstrument(const scoreInstrument& instr) {
  if (fMidiWriter) {
    if (fMidiInstruments.count(instr.fID)) {
      ...
      fMidiWriter->newInstrument(instr.fName);
  }
}
...
void midicontextvisitor::visitEnd(S_midi_instrument& elt) {
  if (!midiInstrument::fID.empty()) {
    ...
    if (fInSound) playMidiInstrument(*this);
  } else
    cerr << "midicontextvisitor: unexpected empty id while visiting "
            "S_midi_instrument" << endl;
}

//________________________________________________________________________
void midicontextvisitor::visitStart(S_divisions& elt) {
  fDivisions = (long)(*elt);
}
void midicontextvisitor::visitEnd(S_measure& elt) {
  fCurrentDate = fEndMeasureDate;
}
void midicontextvisitor::visitEnd(S_transpose& elt) {
  fTranspose = fChromatic + (fOctaveChange * 12);
}
...
//________________________________________________________________________
void midicontextvisitor::visitEnd(S_part& elt) {
  if (fMidiWriter) fMidiWriter->endPart(fCurrentDate);
}
void midicontextvisitor::visitEnd(S_note& elt) { playNote(*this); }
void midicontextvisitor::visitStart(S_score_part& elt) {
  fCurrentPartID = elt->getAttributeValue("id");
}
...
void midicontextvisitor::visitEnd(S_score_instrument& elt) {
  ...
  // the corresponding data is stored for future use, actually at the beginning
  // of a part
  if (!fCurrentPartID.empty())
    fScoreInstruments.insert(
        pair<string, scoreInstrument>(fCurrentPartID, *this));
  else
    cerr << "midicontextvisitor: unexpected empty part id while visiting "
            "S_score_instrument" << endl;
}
